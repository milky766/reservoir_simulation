...
for pos in test_initial_positions_angles:
    esn_output = esn.run(np.array([pos] * steps))           # run メソッドを使って、各初期位置 pos からの自律走行をシミュレートします。入力を与えず、前の出力を次の入力として使用し続けることです
    esn_trajectory = []
    esn_torque_seq = []
    q_dot = np.zeros(2)
    for q in esn_output:
        torque = pd_control(target_angles, q, q_dot, KP, KD)
        esn_trajectory.append(q)
        esn_torque_seq.append(torque)
        q_dot = q_dot + dynamics(q, q_dot, torque) * dt
    esn_trajectories.append(np.array(esn_trajectory))
